...
{{-- ===== FLOATING CART BUTTON (Dari temanmu, dengan route yang sudah diperbaiki) ===== --}}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-28">
    ...
    {{-- Brand Mark (Dari HEAD, logo tidak di-hidden) --}}
...
@push('scripts')
<script>
// Karena kamu pakai AJAX store cart, pastikan meta tag csrf ada di layouts.app
...
function loadCartBadge() {
    fetch('/customer/cart/data', { headers: { 'X-CSRF-TOKEN': CSRF } })
    .then(r => r.json())
    .then(res => {
    ...
    .catch(e => console.log('Belum ada cart atau error fetch'));
}
...
function addToCart(menuId, name, price, image) {
    ...
    // Di web.php kita punya POST /customer/cart untuk store
    fetch("{{ route('customer.cart.store') }}", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF },
        body: JSON.stringify({ menu_id: menuId, quantity: 1, note: '' })
    })
    .then(r => r.json())
    .then(res => {
        if (res.success) {
            // Update qty akurat dari server
            updateBadge(res.cart_count, oldPrice + price);
    ...
    .catch(() => {
        ...
        localStorage.setItem('cart_count', oldQty);
        localStorage.setItem('cart_price', oldPrice);
        alert('Gagal menambahkan ke keranjang! Cek koneksi Anda.');
    })
    .finally(() => {
        setTimeout(() => {
            btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // (Abaikan jika endpoint cart/data belum kamu buat di controller, biarkan pakai optimistic)
    // loadCartBadge(); 
});
</script>
@endpush